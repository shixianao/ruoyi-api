...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements ISysDeptService {
	...
	@Override
    public Map<Long, SysDept> selectDeptMapByIds(Set<Long> ids) {
		if (CollectionUtils.isEmpty(ids)) {
			return new HashMap<>();
		}
		return this.getBaseMapper().selectBatchIds(ids).stream().collect(Collectors.toMap(SysDept::getDeptId, u -> u));
	}
    ...
    public int selectNormalChildrenDeptById(Long deptId) {
        return ((Long) count(new LambdaQueryWrapper<SysDept>()
                .eq(SysDept::getStatus, 0)
                .apply("find_in_set({0}, ancestors)", deptId))).intValue();
    }
