...
public class ExcelUtil<T>
{
    ...
    {
        return importExcel(StrUtil.EMPTY, is, 0);
    }
    
    public List<T> importExcel(InputStream is, int headerRowIndex) throws Exception
    {
        return importExcel(StrUtil.EMPTY, is, headerRowIndex);
    }
     ...
     * @return 转换后集合
     */
    public List<T> importExcel(String sheetName, InputStream is, int headerRowIndex) throws Exception
    {
        ...
        if (rows > 0)
        {
            ...
            // 获取表头
            Row heard = sheet.getRow(headerRowIndex);
    ...
    public AjaxResult exportExcel(List<T> list, String sheetName, int headerRowIndex)
    {
        this.init(list, sheetName, Type.EXPORT);
        return exportExcel(headerRowIndex);
    }
    ...
    public void exportExcel(List<T> list, int headerRowIndex, XSSFWorkbook workbook)
    {
    	this.type = Type.EXPORT;
    	this.maxHeight = (short) 400;
        if (list == null)
        {
            list = new ArrayList<T>();
        }
        ...
        try
        {
            ...
            for (int index = 0; index <= sheetNo; index++)
            {
                ...
                {
                	Row row = sheet.getRow(headerRowIndex + i + 1 - startNo);
                	if (null == row) {
                		row = sheet.createRow(headerRowIndex + i + 1 - startNo);
                	}
                    // 得到导出对象.
                    T vo = (T) list.get(i);
                    for (Map.Entry<Integer, Field> entry : fieldsMap.entrySet())
                    {
                        ...
                        // 设置实体类私有属性可访问
                        field.setAccessible(true);
                        this.addCell(excel, row, vo, field, entry.getKey());
                    }
                }
            }
        }
     ...
     * @return 结果
     */
    public AjaxResult exportExcel(int headerRowIndex)
    {
        ...
        try
        {
            ...
            for (int index = 0; index <= sheetNo; index++)
            {
                ...
                {
                    fillExcelData(index, row, headerRowIndex);
                    addStatisticsRow();
                }
            }
     ...
     * @param row 单元格行
     */
    public void fillExcelData(int index, Row row, int headerRowIndex)
    {
        int startNo = index * sheetSize;
        int endNo = Math.min(startNo + sheetSize, list.size());
        for (int i = startNo; i < endNo; i++)
    ...
    {
        filename = filename + "_" + UUID.randomUUID().toString().replaceAll("-", "") + ".xlsx";
        return filename;
    }
    ...
    {
        String downloadPath = ErpConfig.getDownloadPath() + filename;
